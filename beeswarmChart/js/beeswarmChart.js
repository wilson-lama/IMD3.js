        ...
        tooltip
            ...
            .style("top", event.pageY + "px").html(`
                <div style="border: thin solid grey; border-radius: 5px; background: darkgrey; padding: 5px">
                    ...
                    <span class="text-center">Revenue: ${d3.format("$,")(
                        d.value
                    )}</span>
                </div>`);
...
class BeeswarmChart {
    ...
    initVis() {
        ...
        document.querySelector("#" + vis.parentElement).append(vis.chart);
        ...
        // Scales and axes
        vis.x = d3.scaleLog().rangeRound([0, vis.width]);

        let revenueMin = d3.min(vis.data, (d) =>
            Number(d.revenue.slice(1).split(",").join(""))
        );
        let revenueMax = d3.max(vis.data, (d) =>
            Number(d.revenue.slice(1).split(",").join(""))
        );
        let revenueScale = d3
            .scaleLinear()
        ...
        let circles = vis.svg
            ...
            .attr("r", (row_data, row_number) => {
                return revenueScale(
                    Number(row_data.revenue.slice(1).split(",").join(""))
                );
            })
